...
export const admin_create_seller = createAsyncThunk(
    'seller/admin_create_seller',
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await api.post('/admin/create-seller', info, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const update_seller_permissions = createAsyncThunk(
    'seller/update_seller_permissions',
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await api.post('/admin/update-seller-permissions', info, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const update_seller_password = createAsyncThunk(
    'seller/update_seller_password',
    async (info, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await api.post('/admin/update-seller-password', info, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const sellerReducer = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(admin_create_seller.fulfilled, (state, { payload }) => {
                state.loader = false;
                state.successMessage = payload.message;
                state.sellers = [...state.sellers, payload.seller];
            })
            .addCase(admin_create_seller.rejected, (state, { payload }) => {
                state.loader = false;
                state.errorMessage = payload.error;
            })
            .addCase(update_seller_permissions.fulfilled, (state, { payload }) => {
                state.loader = false;
                state.successMessage = payload.message;
            })
            .addCase(update_seller_permissions.rejected, (state, { payload }) => {
                state.loader = false;
                state.errorMessage = payload.error;
            })
            .addCase(update_seller_password.fulfilled, (state, { payload }) => {
                state.loader = false;
                state.successMessage = payload.message;
            })
            .addCase(update_seller_password.rejected, (state, { payload }) => {
                state.loader = false;
                state.errorMessage = payload.error;
            })
